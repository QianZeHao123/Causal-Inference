...
library(ggplot2)
data %>% 
  select(DemPct, DPctNxt) %>% 
  gather(key = "variable", value = "value") %>% 
  ggplot(aes(x = value)) +
  geom_histogram(bins = 30, fill = "blue", alpha = 0.7) +
  facet_wrap(~variable, scales = "free_x") +
  labs(title = "Histrogram of Democratic Vote Turnout Rate")
```
...
#   time.optimize.ssr = 1930:2008,  # Ensure this period is covered by your data
...
```{r}
data.glm = data.synth.all %>%
  mutate(TreatmentPos = ifelse(ElcSwing > 5, 1, 0)) %>%
  mutate(TreatmentNeg = ifelse(ElcSwing < -5, 1, 0))
model.psm = glm(TreatmentNeg ~ TreatmentPos +
    DemWin + DWinPrv + DSpndPct + DDonaPct +
    GovWkPct + UrbanPct, data = data.glm,
    family = "binomial")
```

## 5.2 Propensity Score Calculation

```{r}
data.glm$pscore = predict(model.psm, type = "response")
```
...
```{r}
library(MatchIt)
matchit_res <- matchit(TreatmentNeg ~ TreatmentPos +
    DemWin + DWinPrv + DSpndPct + DDonaPct +
      GovWkPct + UrbanPct,
    data = data.glm,
