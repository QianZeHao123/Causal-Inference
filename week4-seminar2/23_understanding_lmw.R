...
d <- sapply(plogis(a0-0.5,0,1), {\(i) rbinom(1,1,i)})
...
#   compare
c('E[Y|D=0]'=mean(y[d%in%0]), 
  'E_w[Y|D=0]'=sum((w*y)[d%in%0]),
  'E[Y|D=1]'=mean(y[d%in%1]), 
  'E_w[Y|D=1]'=sum((w*y)[d%in%1])) |> round(3)
...
plot(w2[d%in%0], w[d%in%0], asp=1, pch=16,
     col=rgb(0,0,1,0.5))
...
plot(mw, type='extrapolation', var='x', data=dat)
...
plot(mw, type='influence', outcome='y', data=dat)
...
oneboot <-
    function(md=dat)
    {
        ...
        w <- lmw(~d+x+a0+a1, treat='d', data=md_,
                 estimand='ATT', method='MRI')$weights
        ess0 <- ess(w[md_$d%in%0])
        n0 <- sum(md_$d%in%0)
        return(c('att'=att, 'n0'=n0, 'ess0'=ess0))
    }
...
for (i in 1:N) {
    if (d[i]%in%1) {
        u[i] <- 1/N1 + t(X[i,]-xbar1) %*% solve(N1*V1+N0*V0) %*% (xbar0-xbar1)
    } else if (d[i]%in%0) {
        u[i] <- 1/N0 + t(X[i,]-xbar0) %*% solve(N1*V1+N0*V0) %*% (xbar1-xbar0)
    } else {
        u[i] <- NA
    }
}
...
par(mar=c(5,5,1,1), las=1)
plot(w, u, asp=1, pch=16, col=grey(0.5, 0.5), 
     xlab='By-hand weights', ylab='Weights from lmw 0.0.1')
