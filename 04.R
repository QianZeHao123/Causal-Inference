...
plot(y0,
     y1,
     asp = 1,
     col = ifelse(d_o %in% 0, 'red3', 'blue3'))
abline(
  a = 0,
  b = 1,
  col = grey(0.2),
...
set.seed(286)
r_a <-
  apply(g, 1, function(i)
    onestratum(s_ = i['s'], d_ = i['d_o'], n_ = i['n']))
r_a
i_a <- unlist(r_a)
...
ps <-
  glm(d_o ~ 1 + as.factor(s), family = binomial('logit')) |> fitted()
...
for (i in 1:4) {
  set.seed(i + 100)
  points(e[, i],
         rep(i, R) + runif(R,-0.3, 0.3),
         pch = 1,
         col = 'lightblue3')
  lines(rep(mean(e[, i], na.rm = T), 2),
        i + c(-1, 1) * 0.4,
        col = 'darkblue',
        lwd = 2)
}
